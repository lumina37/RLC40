...
#include "tlct2/config/common/enums.hpp"
#include "tlct2/config/concepts/factory.hpp"
#include "tlct2/helper.hpp"
...
public:
    ...
    // Constructor
    TLCT2_API inline ConfigMap() : map_(){};
    TLCT2_API inline ConfigMap(const ConfigMap& rhs) = default;
    TLCT2_API inline ConfigMap& operator=(const ConfigMap& rhs) = default;
    TLCT2_API inline ConfigMap(ConfigMap&& rhs) noexcept = default;
    TLCT2_API inline ConfigMap& operator=(ConfigMap&& rhs) noexcept = default;
    TLCT2_API explicit inline ConfigMap(TMap&& cfg_map) noexcept : map_(std::move(cfg_map)){};

    // Initialize from
    [[nodiscard]] TLCT2_API static inline ConfigMap fromFstream(std::ifstream&& ifs);
    [[nodiscard]] TLCT2_API static inline ConfigMap fromPath(std::string_view path);

    // Const methods
    [[nodiscard]] TLCT2_API inline bool isEmpty() const noexcept;
    [[nodiscard]] TLCT2_API inline int getPipelineType() const noexcept;

    template <typename Tv, _hp::cestring key>
    [[nodiscard]] TLCT2_API inline Tv get() const;

    template <typename Tv, _hp::cestring key, Tv default_val>
        requires std::is_trivially_copyable_v<Tv>
    [[nodiscard]] TLCT2_API inline Tv get() const noexcept;
    ...
    [[nodiscard]] TLCT2_API inline Tv get(const std::string& key) const;

    template <typename Tv>
        requires std::is_trivially_copyable_v<Tv>
    [[nodiscard]] TLCT2_API inline Tv get(const std::string& key, Tv default_val) const noexcept;

    template <typename Tf, Tf default_factory, typename Tv>
        requires concepts::is_factory_of<Tf, Tv>
    [[nodiscard]] TLCT2_API inline Tv get(const std::string& key) const noexcept;

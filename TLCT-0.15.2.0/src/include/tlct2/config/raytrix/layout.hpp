...
public:
    ...
    // Constructor
    TLCT2_API inline Layout() noexcept
    ...
    TLCT2_API inline Layout(const Layout& rhs) noexcept = default;
    TLCT2_API inline Layout& operator=(const Layout& rhs) noexcept = default;
    TLCT2_API inline Layout(Layout&& rhs) noexcept = default;
    TLCT2_API inline Layout& operator=(Layout&& rhs) noexcept = default;
    TLCT2_API inline Layout(cv::Point2d left_top, bool is_out_shift, double x_unit_shift, double y_unit_shift,
    ...
    // Initialize from
    [[nodiscard]] TLCT2_API static inline Layout fromCalibAndSpecConfig(const TCalibConfig& calib_cfg,
                                                                       const TSpecificConfig& spec_cfg);

    // Non-const methods
    TLCT2_API inline Layout& upsample(int factor) noexcept;

    // Const methods
    [[nodiscard]] TLCT2_API inline int getImgWidth() const noexcept { return imgsize_.width; };
    [[nodiscard]] TLCT2_API inline int getImgHeight() const noexcept { return imgsize_.height; };
    [[nodiscard]] TLCT2_API inline cv::Size getImgSize() const noexcept { return imgsize_; };
    [[nodiscard]] TLCT2_API inline int getMIType(int row, int col) const noexcept;
    [[nodiscard]] TLCT2_API inline int getMIType(cv::Point index) const noexcept;
    [[nodiscard]] TLCT2_API inline double getDiameter() const noexcept { return diameter_; };
    [[nodiscard]] TLCT2_API inline double getRadius() const noexcept { return radius_; };
    [[nodiscard]] TLCT2_API inline double getRotation() const noexcept { return rotation_; };
    [[nodiscard]] TLCT2_API inline int getUpsample() const noexcept { return upsample_; };
    [[nodiscard]] TLCT2_API inline int getMIRows() const noexcept { return mirows_; };
    [[nodiscard]] TLCT2_API inline int getMICols(const int row) const noexcept { return micols_[row % micols_.size()]; };
    [[nodiscard]] TLCT2_API inline int getMIMaxCols() const noexcept { return std::max(micols_[0], micols_[1]); };
    [[nodiscard]] TLCT2_API inline int getMIMinCols() const noexcept { return std::min(micols_[0], micols_[1]); };
    [[nodiscard]] TLCT2_API inline cv::Point2d getMICenter(int row, int col) const noexcept;
    [[nodiscard]] TLCT2_API inline cv::Point2d getMICenter(cv::Point index) const noexcept;
    [[nodiscard]] TLCT2_API inline bool isOutShift() const noexcept { return is_out_shift_; };
    [[nodiscard]] TLCT2_API inline int isOutShiftSgn() const noexcept { return _hp::sgn(isOutShift()); };

    TLCT2_API inline void processInto(const cv::Mat& src, cv::Mat& dst) const;

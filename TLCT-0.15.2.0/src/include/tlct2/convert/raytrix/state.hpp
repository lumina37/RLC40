...
namespace tcfg = tlct2::cfg::raytrix;
...
public:
    ...
    TLCT2_API inline State(State&& rhs) noexcept = default;
    TLCT2_API inline State& operator=(State&& rhs) noexcept = default;
    TLCT2_API inline State(TLayout&& layout, TSpecificConfig&& spec_cfg, TMIs&& mis,
    ...
    // Initialize from
    [[nodiscard]] TLCT2_API static inline State fromParamCfg(const TParamConfig& param_cfg);

    // Non-const methods
    TLCT2_API inline void update(const cv::Mat& src);

...
template <tlct2::cfg::concepts::CLayout TLayout_>
...
public:
    ...
    TLCT2_API inline NearNeighbors_(const NearNeighbors_& rhs) noexcept = default;
    TLCT2_API inline NearNeighbors_& operator=(const NearNeighbors_& rhs) noexcept = default;
    TLCT2_API inline NearNeighbors_(NearNeighbors_&& rhs) noexcept = default;
    TLCT2_API inline NearNeighbors_& operator=(NearNeighbors_&& rhs) noexcept = default;
    TLCT2_API inline NearNeighbors_(cv::Point self_idx, TIndices indices, cv::Point2d self_pt, TPoints points) noexcept
        : self_idx_(self_idx), indices_(indices), self_pt_(self_pt), points_(points){};

    // Initialize from
    [[nodiscard]] TLCT2_API static inline NearNeighbors_ fromLayoutAndIndex(const TLayout& layout,
                                                                           cv::Point index) noexcept;

    // Const methods
    [[nodiscard]] TLCT2_API inline cv::Point getSelfIdx() const noexcept { return self_idx_; };
    [[nodiscard]] TLCT2_API inline cv::Point2d getSelfPt() const noexcept { return self_pt_; };

    // Left is 0. Clockwise.
    [[nodiscard]] TLCT2_API inline bool hasNeighbor(const Direction direction) const noexcept
    {
        return indices_[(int)direction].x != DEFAULT_INDEX;
    };
    [[nodiscard]] TLCT2_API inline cv::Point getNeighborIdx(const Direction direction) const noexcept
    {
        return indices_[(int)direction];
    };
    [[nodiscard]] TLCT2_API inline cv::Point2d getNeighborPt(const Direction direction) const noexcept
    {
        return points_[(int)direction];
    };
    [[nodiscard]] TLCT2_API static inline cv::Point2d getUnitShift(const Direction direction) noexcept
    {
...
template <tlct2::cfg::concepts::CLayout TLayout>
...
public:
    ...
    TLCT2_API inline FarNeighbors_(const FarNeighbors_& rhs) noexcept = default;
    TLCT2_API inline FarNeighbors_& operator=(const FarNeighbors_& rhs) noexcept = default;
    TLCT2_API inline FarNeighbors_(FarNeighbors_&& rhs) noexcept = default;
    TLCT2_API inline FarNeighbors_& operator=(FarNeighbors_&& rhs) noexcept = default;
    TLCT2_API inline FarNeighbors_(cv::Point self_idx, TIndices indices, cv::Point2d self_pt, TPoints points) noexcept
        : self_idx_(self_idx), indices_(indices), self_pt_(self_pt), points_(points){};

    // Initialize from
    [[nodiscard]] TLCT2_API static inline FarNeighbors_ fromLayoutAndIndex(const TLayout& layout,
                                                                          cv::Point index) noexcept;

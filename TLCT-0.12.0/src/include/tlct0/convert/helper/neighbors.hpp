...
template <typename TLayout_, int LEN_TYPE_NUM_>
    requires tlct0::cfg::concepts::CLayout<TLayout_>
class NeighborIdx_
{
public:
    ...
    // Constructor
    TLCT0_API inline NeighborIdx_& operator=(const NeighborIdx_& rhs) noexcept = default;
    TLCT0_API inline NeighborIdx_(const NeighborIdx_& rhs) noexcept = default;
    TLCT0_API inline NeighborIdx_& operator=(NeighborIdx_&& rhs) noexcept = default;
    TLCT0_API inline NeighborIdx_(NeighborIdx_&& rhs) noexcept = default;
    TLCT0_API inline NeighborIdx_(cv::Point left, cv::Point right, cv::Point upleft, cv::Point upright,
    ...
    // Initialize from
    [[nodiscard]] TLCT0_API static inline NeighborIdx_ fromLayoutAndIndex(const TLayout& layout,
                                                                         const cv::Point index) noexcept;

    // Const methods
    [[nodiscard]] TLCT0_API inline bool hasLeft() const noexcept { return left_.x != DEFAULT_INDEX; }
    [[nodiscard]] TLCT0_API inline bool hasRight() const noexcept { return right_.x != DEFAULT_INDEX; }
    [[nodiscard]] TLCT0_API inline bool hasUpLeft() const noexcept { return upleft_.x != DEFAULT_INDEX; }
    [[nodiscard]] TLCT0_API inline bool hasUpRight() const noexcept { return upright_.x != DEFAULT_INDEX; }
    [[nodiscard]] TLCT0_API inline bool hasDownLeft() const noexcept { return downleft_.x != DEFAULT_INDEX; }
    [[nodiscard]] TLCT0_API inline bool hasDownRight() const noexcept { return downright_.x != DEFAULT_INDEX; }
    [[nodiscard]] TLCT0_API inline cv::Point getLeft() const noexcept { return left_; };
    [[nodiscard]] TLCT0_API inline cv::Point getRight() const noexcept { return right_; };
    [[nodiscard]] TLCT0_API inline cv::Point getUpLeft() const noexcept { return upleft_; };
    [[nodiscard]] TLCT0_API inline cv::Point getUpRight() const noexcept { return upright_; };
    [[nodiscard]] TLCT0_API inline cv::Point getDownLeft() const noexcept { return downleft_; };
    [[nodiscard]] TLCT0_API inline cv::Point getDownRight() const noexcept { return downright_; };

    template <Direction direction>
    [[nodiscard]] TLCT0_API inline cv::Point2d getNeighbor() const noexcept
    {
    ...
    template <Direction direction>
    [[nodiscard]] TLCT0_API inline bool hasNeighbor() const noexcept
    {
        if constexpr (direction == Direction::LEFT)
            return hasLeft();
        if constexpr (direction == Direction::RIGHT)
...
template <typename TLayout, int LEN_TYPE_NUM>
    requires tlct0::cfg::concepts::CLayout<TLayout>

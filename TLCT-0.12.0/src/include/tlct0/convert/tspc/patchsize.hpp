...
#include "tlct0/convert/helper/neighbors.hpp"
#include "tlct0/convert/helper/roi.hpp"
#include "tlct0/convert/helper/wrapper.hpp"
#include "tlct0/convert/tspc/state.hpp"

namespace tlct0::cvt::tspc {

namespace rgs = std::ranges;

namespace _hp {

using namespace tlct0::cvt::_hp;
namespace tcfg = tlct0::cfg::tspc;
...
static inline int estimatePatchsizeOverFullMatch(const cfg::tspc::Layout& layout, const cv::Mat& gray_src,
                                                 ...
                                                 const double ksize, const double min_psize_factor) noexcept
{
    const cv::Point2d curr_center = layout.getMICenter(index);
                                    ...
                                    const cv::Mat& prev_psizes, const cv::Point index, const double min_psize_factor)
{
    ...
    const int psize = estimatePatchsizeOverFullMatch(layout, gray_src, index, neighbors, ksize, min_psize_factor);
...
TLCT0_API inline cv::Mat estimatePatchsizes(State& state)
{
    ...
    for (const int row : rgs::views::iota(0, layout.getMIRows())) {
        for (const int col : rgs::views::iota(0, layout.getMICols(row))) {
            ...
            const int psize =
                _hp::estimatePatchsize(layout, state.gray_src_, psizes, prev_psizes, index, state.min_psize_factor_);

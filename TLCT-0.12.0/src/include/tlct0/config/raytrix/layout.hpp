...
public:
    ...
    // Constructor
    TLCT0_API inline Layout() noexcept
        : left_top_(), is_out_shift_(), x_unit_shift_(), y_unit_shift_(), imgsize_(), mirows_(), micols_(), diameter_(),
          radius_(), rotation_(), upsample_(1){};
    TLCT0_API inline Layout& operator=(const Layout& rhs) noexcept = default;
    TLCT0_API inline Layout(const Layout& rhs) noexcept = default;
    ...
    // Non-const methods
    TLCT0_API inline Layout& upsample(const int factor) noexcept;
    TLCT0_API inline Layout& transpose() noexcept;

    // Const methods
    [[nodiscard]] TLCT0_API inline int getImgWidth() const noexcept { return imgsize_.width; };
    [[nodiscard]] TLCT0_API inline int getImgHeight() const noexcept { return imgsize_.height; };
    [[nodiscard]] TLCT0_API inline cv::Size getImgSize() const noexcept { return imgsize_; };
    [[nodiscard]] TLCT0_API inline int getMIType(const int row, const int col) const noexcept;
    [[nodiscard]] TLCT0_API inline int getMIType(const cv::Point index) const noexcept;
    [[nodiscard]] TLCT0_API inline double getDiameter() const noexcept { return diameter_; };
    [[nodiscard]] TLCT0_API inline double getRadius() const noexcept { return radius_; };
    [[nodiscard]] TLCT0_API inline double getRotation() const noexcept { return rotation_; };
    [[nodiscard]] TLCT0_API inline int getUpsample() const noexcept { return upsample_; };
    [[nodiscard]] TLCT0_API inline cv::Point2d getMICenter(const int row, const int col) const noexcept;
    [[nodiscard]] TLCT0_API inline cv::Point2d getMICenter(const cv::Point index) const noexcept;
    [[nodiscard]] TLCT0_API inline int getMIRows() const noexcept { return mirows_[0]; };
    [[nodiscard]] TLCT0_API inline int getMICols(const int row) const noexcept
    {
        return micols_[row % micols_.channels];
    };
    [[nodiscard]] TLCT0_API inline int getMIMaxCols() const noexcept { return std::max(micols_[0], micols_[1]); };
    [[nodiscard]] TLCT0_API inline int getMIMinCols() const noexcept { return std::min(micols_[0], micols_[1]); };
    [[nodiscard]] TLCT0_API inline bool isOutShift() const noexcept { return is_out_shift_; };
    [[nodiscard]] TLCT0_API inline int isOutShiftSgn() const noexcept { return (int)(isOutShift()) * 2 - 1; };

    TLCT0_API inline void procImg_(const cv::Mat& src, cv::Mat& dst) const;
    [[nodiscard]] TLCT0_API inline cv::Mat procImg(const cv::Mat& src) const;

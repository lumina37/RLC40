...
public:
    // Constructor
    TLCT0_API inline CommonParamConfig() noexcept : views_(), range_(), src_pattern_(), dst_pattern_(){};
    TLCT0_API inline CommonParamConfig& operator=(const CommonParamConfig& rhs) noexcept = default;
    TLCT0_API inline CommonParamConfig(const CommonParamConfig& rhs) noexcept = default;
    ...
    // Initialize from
    [[nodiscard]] TLCT0_API static inline CommonParamConfig fromConfigMap(const ConfigMap& cfg_map);

    // Const methods
    [[nodiscard]] TLCT0_API inline int getViews() const noexcept { return views_; };
    [[nodiscard]] TLCT0_API inline cv::Range getRange() const noexcept { return range_; };
    [[nodiscard]] TLCT0_API inline const std::string& getSrcPattern() const noexcept { return src_pattern_; };
    [[nodiscard]] TLCT0_API inline const std::string& getDstPattern() const noexcept { return dst_pattern_; };

    [[nodiscard]] TLCT0_API inline fs::path fmtSrcPath(int i) const noexcept;
    [[nodiscard]] TLCT0_API inline fs::path fmtDstPath(int i) const noexcept;
...
public:
    ...
    // Constructor
    TLCT0_API inline ParamConfig_() noexcept : calib_cfg_(), common_cfg_(), imgsize_(){};
    TLCT0_API inline ParamConfig_& operator=(const ParamConfig_& rhs) noexcept = default;
    TLCT0_API inline ParamConfig_(const ParamConfig_& rhs) noexcept = default;
    TLCT0_API inline ParamConfig_& operator=(ParamConfig_&& rhs) noexcept = default;
    TLCT0_API inline ParamConfig_(ParamConfig_&& rhs) noexcept = default;
    TLCT0_API inline ParamConfig_(TCalibConfig calib_cfg, CommonParamConfig common_cfg, cv::Size imgsize) noexcept
        : calib_cfg_(calib_cfg), common_cfg_(std::move(common_cfg)), imgsize_(imgsize){};

    // Initialize from
    [[nodiscard]] TLCT0_API static inline ParamConfig_ fromConfigMap(const ConfigMap& cfg_map);

    // Const methods
    [[nodiscard]] TLCT0_API inline const TCalibConfig& getCalibCfg() const noexcept { return calib_cfg_; };
    [[nodiscard]] TLCT0_API inline const CommonParamConfig& getCommonCfg() const noexcept { return common_cfg_; };

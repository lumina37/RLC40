...
int main(int argc, char* argv[])
{
    ...
    if (cfg_map.getPipelineType() == tlct1::PipelineType::TLCT1) {
        ...
        for (const int row : rgs::views::iota(0, layout.getMIRows())) {
            for (const int col : rgs::views::iota(0, layout.getMICols(row))) {
                ...
                cv::circle(resized_img, center, tlct1::_hp::iround(layout.getRadius()), {0, 0, 255}, 1, cv::LINE_AA);
            }
        }
        for (const int col : rgs::views::iota(0, layout.getMICols(0))) {
            const auto center = layout.getMICenter(0, col);
            cv::circle(resized_img, center, tlct1::_hp::iround(layout.getRadius()), {255, 0, 0}, 1, cv::LINE_AA);
        }

        const cv::Scalar base_color{0, 255.0 / 6, 255.0 / 6};
        auto neighbors = tn::Neighbors::fromLayoutAndIndex(layout, {4, 3});
        for (const auto direction : tlct1::DIRECTIONS) {
            cv::circle(resized_img, neighbors.getNeighborPt(direction), tlct1::_hp::iround(layout.getRadius()),
        ...
        for (const int row : rgs::views::iota(0, layout.getMIRows())) {
            for (const int col : rgs::views::iota(0, layout.getMICols(row))) {
                ...
                cv::circle(resized_img, center, tlct1::_hp::iround(layout.getRadius()),

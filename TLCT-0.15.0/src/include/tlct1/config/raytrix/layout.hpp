...
public:
    ...
    // Constructor
    TLCT1_API inline Layout() noexcept
    ...
    TLCT1_API inline Layout(const Layout& rhs) noexcept = default;
    TLCT1_API inline Layout& operator=(const Layout& rhs) noexcept = default;
    TLCT1_API inline Layout(Layout&& rhs) noexcept = default;
    TLCT1_API inline Layout& operator=(Layout&& rhs) noexcept = default;
    TLCT1_API inline Layout(cv::Point2d left_top, bool is_out_shift, double x_unit_shift, double y_unit_shift,
    ...
    // Const methods
    [[nodiscard]] TLCT1_API inline int getImgWidth() const noexcept { return imgsize_.width; };
    [[nodiscard]] TLCT1_API inline int getImgHeight() const noexcept { return imgsize_.height; };
    [[nodiscard]] TLCT1_API inline cv::Size getImgSize() const noexcept { return imgsize_; };
    [[nodiscard]] TLCT1_API inline int getMIType(int row, int col) const noexcept;
    [[nodiscard]] TLCT1_API inline int getMIType(cv::Point index) const noexcept;
    [[nodiscard]] TLCT1_API inline double getDiameter() const noexcept { return diameter_; };
    [[nodiscard]] TLCT1_API inline double getRadius() const noexcept { return radius_; };
    ...
    [[nodiscard]] TLCT1_API inline cv::Point2d getMICenter(cv::Point index) const noexcept;
    [[nodiscard]] TLCT1_API inline bool isOutShift() const noexcept { return is_out_shift_; };
    [[nodiscard]] TLCT1_API inline int isOutShiftSgn() const noexcept { return _hp::sgn(isOutShift()); };

    TLCT1_API inline void processInto(const cv::Mat& src, cv::Mat& dst) const;

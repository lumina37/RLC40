...
public:
    // Constructor
    TLCT1_API inline CalibConfig() noexcept
        : diameter_(), rotation_(), left_top_(), right_top_(), left_bottom_(), right_bottom_(){};
    TLCT1_API inline CalibConfig(const CalibConfig& rhs) noexcept = default;
    TLCT1_API inline CalibConfig& operator=(const CalibConfig& rhs) noexcept = default;
    TLCT1_API inline CalibConfig(CalibConfig&& rhs) noexcept = default;
    TLCT1_API inline CalibConfig& operator=(CalibConfig&& rhs) noexcept = default;
    TLCT1_API inline CalibConfig(double diameter, double rotation, cv::Point2d left_top, cv::Point2d right_top,
    ...
    // Initialize from
    [[nodiscard]] TLCT1_API static inline CalibConfig fromXMLDoc(const pugi::xml_document& doc);
    [[nodiscard]] TLCT1_API static inline CalibConfig fromXMLPath(std::string_view path);
    ...
    [[nodiscard]] TLCT1_API inline cv::Point2d getLeftTop() const noexcept { return left_top_; };
    [[nodiscard]] TLCT1_API inline cv::Point2d getRightTop() const noexcept { return right_top_; };
    [[nodiscard]] TLCT1_API inline cv::Point2d getLeftBottom() const noexcept { return left_bottom_; };
    [[nodiscard]] TLCT1_API inline cv::Point2d getRightBottom() const noexcept { return right_bottom_; };

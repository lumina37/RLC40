...
public:
    ...
    TLCT1_API inline GenericParamConfig(const GenericParamConfig& rhs) = default;
    TLCT1_API inline GenericParamConfig& operator=(const GenericParamConfig& rhs) = default;
    ...
    // Initialize from
    [[nodiscard]] TLCT1_API static inline GenericParamConfig fromConfigMap(const ConfigMap& cfg_map);

    // Const methods
    [[nodiscard]] TLCT1_API inline int getViews() const noexcept { return views_; };
    [[nodiscard]] TLCT1_API inline cv::Range getRange() const noexcept { return range_; };
    [[nodiscard]] TLCT1_API inline const std::string& getSrcPattern() const noexcept { return src_pattern_; };
    [[nodiscard]] TLCT1_API inline const std::string& getDstPattern() const noexcept { return dst_pattern_; };

    [[nodiscard]] TLCT1_API inline fs::path fmtSrcPath(int i) const noexcept;
    [[nodiscard]] TLCT1_API inline fs::path fmtDstPath(int i) const noexcept;
